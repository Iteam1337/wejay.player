...
Spotify.prototype.search = function(query) {
  ...
  var search = new this.app.Search(query, 2, 10);
  search.execute(function(err, searchResult) {
    if (err) {
      console.error('search error occured', err);
      deferred.reject();
    }
...
Spotify.prototype.play = function(uri) {
  var track = this.app.createFromLink(uri);
  console.log(track);
